...
def event_list(model_name):
    ...
    # reverse_sort = model_config['reverse_sort']

    kwargs = _get_args()
    event_filter = kwargs.get('filter', '')
    page = kwargs.get('page', 1)
    sort = kwargs.get('sort', model_pk)
    ...
    for header in headers:
        ...
        images = url_for('kadi.static', filename='images')
    ...
    # the table content
    event_rows = [
        (j, [formats[fn].format(getattr(ke, fn)) for fn in field_names])  # why is j needed?
